...
class APISearchPipeline(SemanticSearchPipeline):
    ...
    def __call__(
        ...
        return_embeddings: bool=False):
        if self.inference_mode:
            return self._predict(queries, max_num_results)
        return self._search(text = queries, max_num_results = max_num_results)
    ...
    def _predict(self, input: str, max_num_results: int):
        ...
        return self._search(text=input, max_num_results=max_num_results, query_embeddings=query_embeddings)

    def _search(self, text: Union[str, List[str]], max_num_results, query_embeddings=None):
